...
const TMPL_MAIN_C_BRIDGE =
    \\#include <stdio.h>
    \\#include <stdint.h>
    \\
    \\/* Point -- interface from main.ts, compiled as a C struct */
    \\typedef struct {
    \\    double x;
    \\    double y;
    \\} Point;
    \\
    \\/* Counter -- C wrappers for the C++ class (see Counter.cpp) */
    \\typedef struct Counter Counter;
    \\Counter* counter_create(int32_t init);
    \\void counter_increment(Counter* self);
    \\int32_t counter_getVal(Counter* self);
...
const TMPL_BRIDGE_CPP =
    \\
    \\// C-compatible bridge
    \\extern "C" {
    \\    Counter* counter_create(int32_t init) { return new Counter(init); }
    \\    void counter_increment(Counter* self) { self->increment(); }
    \\    int32_t counter_getVal(Counter* self) { return self->getVal(); }
    \\    void counter_destroy(Counter* self) { delete self; }
    \\}
    \\
;

const TMPL_BUILD_ZIG_MIXED =
    \\const std = @import("std");
    \\
    \\pub fn build(b: *std.Build) void {
    \\    const target = b.standardTargetOptions(.{});
    ...
    \\    });
    \\
    \\    // C++ class (generated by zigtsc from main.ts)
    \\    mod.addCSourceFiles(.{
    \\        .root = b.path("src"),
    \\        .files = &.{"Counter.cpp"},
...
fn cmdInit(io: std.Io, allocator: std.mem.Allocator, args: []const []const u8) !void {
    ...
    if (ts) {
        ...
        try dir.writeFile(io, .{ .sub_path = "main.ts", .data = TMPL_MAIN_TS });

        // Run zigtsc to generate C++ class files in src/.
        const ts_path = try std.fmt.allocPrint(allocator, "{s}/main.ts", .{name});
        defer allocator.free(ts_path);
        const cpp_out = try std.fmt.allocPrint(allocator, "{s}/src", .{name});
        defer allocator.free(cpp_out);
        exec(io, &.{ "zigtsc", ts_path, "-target", "cpp", cpp_out }) catch {
            std.debug.print("error: zigtsc failed — install: https://zigtsc.nathanjmorton.com\n", .{});
            return error.CommandFailed;
        };

        // Replace generated main.cpp with a C entrypoint + bridge.
        {
            var src_dir = try dir.openDir(io, "src", .{});
            defer src_dir.close(io);
            src_dir.deleteFile(io, "main.cpp") catch {};

            // Append extern "C" bridge wrappers to Counter.cpp.
            const counter_cpp = src_dir.readFileAlloc(io, "Counter.cpp", allocator, .unlimited) catch {
                std.debug.print("error: zigtsc did not generate Counter.cpp\n", .{});
                return error.CommandFailed;
            };
            defer allocator.free(counter_cpp);
            const updated = try std.mem.concat(allocator, u8, &.{ counter_cpp, TMPL_BRIDGE_CPP });
            defer allocator.free(updated);
            try src_dir.writeFile(io, .{ .sub_path = "Counter.cpp", .data = updated });

            // Write C entrypoint.
            try src_dir.writeFile(io, .{ .sub_path = "main.c", .data = TMPL_MAIN_C_BRIDGE });
        }
