...
# Release zigc: bump version, commit, tag, and push.
# CI handles building, creating the GitHub release, and updating Homebrew.
#
# Usage: ./scripts/release.sh [<version>]
#   No arg  → auto-bump minor (e.g. 0.3.0 → 0.4.0)
#   With arg → use explicit version

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"

# Read current version from src/lib.zig
CURRENT=$(grep -o 'const VERSION = "[^"]*"' "$REPO_ROOT/src/lib.zig" | grep -o '[0-9][0-9.]*')
...
if [[ ! "$VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  echo "error: version must be semver (e.g. 0.4.0)" >&2
  exit 1
fi
...
if [[ "$(uname)" == "Darwin" ]]; then
  sed -i '' "s/const VERSION = \".*\";/const VERSION = \"${VERSION}\";/" "$REPO_ROOT/src/lib.zig"
else
  sed -i "s/const VERSION = \".*\";/const VERSION = \"${VERSION}\";/" "$REPO_ROOT/src/lib.zig"
fi

git -C "$REPO_ROOT" add src/lib.zig
